...
df_weather['z_scores'] = (data['rainfall_obs'] - median) / mad
...
for idx in combined_outliers.index:
    ...
    if obs_value > histo_value + meanDistance:
        df_weather.loc[idx, 'outlier'] = True

outliers = df_weather[df_weather['outlier']]
"""
...
# col containing true if distance outlier
df_weather['outlier_distance'] = False
for idx in distance_outliers.index:
    df_weather.loc[idx, 'outlier_distance'] = True

# outlier total keeps the two types of outlier
outliers_total = df_weather[(df_weather['outlier'] == True) | (df_weather['outlier_distance'] == True)]

# export the outliers based on the distance analysis in a csv file
with open('outliers.csv', 'w', newline='') as f:
    outliers_total.to_csv(f, index=True)